...
public class BoardDAO {		
	...
	public BoardVO getBoard(int no, boolean hitAdd) {
		...
		String sql ="select no,title,writer,regdate,content,hit,ip,fname,pwd from board where no=?";
		try {
			Connection conn = ConnectionProvider.getConnection();
			PreparedStatement pstmt = conn.prepareStatement(sql);		
			pstmt.setInt(1, no);
			...
			if(rs.next()) {
				b=new BoardVO(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getDate(4), rs.getString(5), rs.getInt(6), rs.getString(7), rs.getString(8),rs.getString(9));
			}
	...
	public ArrayList<BoardVO> listAll() {
		...
		try {
			...
			while(rs.next()) {
				list.add(new BoardVO(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getDate(4), rs.getString(5), rs.getInt(6), rs.getString(7), rs.getString(8), rs.getString(9)));
			}
	...
	public int insertBoard(BoardVO b) {
		int re = -1;
		String sql = "insert into board(no, title, writer, regdate, content, hit, ip, fname, pwd) values(seq_board.nextval,?,?,sysdate,?,0,?,?,?)";
		try {
			Connection conn = ConnectionProvider.getConnection();
			PreparedStatement pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, b.getTitle());
